...
export class ChartJsElement extends HTMLElement {
    ...
    _parseRemovals(mutationList) {
        ...
        const datasetsWithRemovedData = mutationList.filter(mut => mut.type === 'childList' && Array.from(mut.removedNodes).some(node => node instanceof HTMLElement) && isDatasetElement(mut.target));
