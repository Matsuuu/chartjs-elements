...
import { LegendMixin } from './mixins/LegendMixin.js';
import { asNumberOrDefault, chartColor, getActiveIndex, isDataElement, isDatasetElement, isSubtitleElement, isTitleElement, waitForInitialization } from './util.js';
 ...
 * @mixes LegendMixin
 * */
export class ChartJsElement extends LegendMixin(HTMLElement) {
    constructor() {
        super();
    ...
    _parseChartData(mutationList = []) {
        ...
        this._parseDatasets();
        this._parseOptions();
        // @ts-ignore TODO: Check how mixin jsdoc works
        this._parseLegend();
    }
    ...
    _parseOptions() {
        ...
        if (titleElement) {
            this.options.plugins.title = {
                ...
                display: true,
                position: titleElement.position ?? 'top',
                padding: asNumberOrDefault(titleElement.padding, 10),
                color: titleElement.color ?? Chart.defaults.color,
                fullSize: titleElement.fullSizeAsBoolean,
                font: {
                    size: asNumberOrDefault(titleElement.size, 12)
                },
            };
        }
        ...
        if (subtitleElement) {
            this.options.plugins.subtitle = {
                text: subtitleElement.text,
                display: true,
                position: subtitleElement.position ?? 'top',
                padding: asNumberOrDefault(subtitleElement.padding, 10),
                color: subtitleElement.color ?? Chart.defaults.color,
                fullSize: subtitleElement.fullSizeAsBoolean,
                font: {
                    size: asNumberOrDefault(subtitleElement.size, 12)
                },
            };
        }
    }
